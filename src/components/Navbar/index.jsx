...
const Navbar = ({ height, menuLinks }) => {
  ...
  return (
    <>
      <AppBar id="simple-usage-app-bar-1" theme="default" fixed height={height}>
        <AppBarNav id="simple-usage-nav-1" aria-label="Navigation">
          <Button id="show-sheet-position" onClick={show}>
        ...
        {NavItems.map(navitems => (
          <AppBarAction
            key={navitems.id}
            id={`navitem-link-${navitems.id}`}
            aria-label={`navlink-${navitems.id}`}
            className={styles.navitems}
          >
            <NavLink theme="default" buttonType="text" to={navitems.link}>
              {navitems.name}
            </NavLink>
          </AppBarAction>
        ))}
      ...
      >
        <List onClick={hide}>
          {menuLinks.map((link, i) => (
            <ListItem id={`menu-item-link-${i}`} key={link.link}>
